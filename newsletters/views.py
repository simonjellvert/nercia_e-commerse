...
def newsletters(request):
    """
    A view for the administrator to see a
    list of previous newsletters, categorized
    """
    ...
    if 'newsletter_category' in request.GET:
        ...
        if category:
            newsletters_list = newsletters_list.filter(
...
def create_newsletter(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            recipients = [
                user_profile.user.email for user_profile in user_profiles
            ]
            ...
            messages.error(
                request,
                'Something went wrong, check your form and try again!'
            )
    else:
        form = NewsletterForm()
